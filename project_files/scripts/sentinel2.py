...
class SentinelPass:
    ...
    def __init__(self,
        ...
        self.sense_time = self._generate_sensing_time(safe_dir)
    ...
    def _generate_sensing_time(self, safe_dir: str):
        '''Parses name of files to create a datetime object for the sensing time'''
        date_str = safe_dir.split('_')[2]
...
class FarmKML:
    ...
    def __init__(self, file_name: str = 'votm_13_Oct_20.kml', outline: str = None, exclusion: str = None):
    ...
    def _get_max_extents(self):
        ...
        buffer_box = box(bound_df[0] + half_mile,
    ...
    def _create_grazeable(self, outline_name, exclusion_name):
        ...
        outline_poly = self.init_gdf[self.init_gdf.Name == outline_name]['geometry'].values[0]
        if exclusion_name:
            exclusion_poly = self.init_gdf[self.init_gdf.Name == exclusion_name]['geometry'].values[0]
...
def sentinel_api(geojson):
    ...
    api = SentinelAPI(os.getenv('SENTINEL_LOGIN'), os.getenv('SENTINEL_PW'), 'https://scihub.copernicus.eu/apihub/')
